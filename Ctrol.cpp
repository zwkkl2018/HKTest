...
int SaveImga(int caminfoId, SYSTEMTIME sys, cv::Mat* rm)
{
	...
	for (int i = 0; i < pCtrol->m_CameraV.size(); i++)
	{
		...
		{
			double value = 0;
			ParaKey saveDataLog(caminfoId, "相机参数", "保存数据");
...
DWORD WINAPI Ctrol::imageProcess(LPVOID pParam)
{
	...
	if (myimgInfo2Dlg.camIndex == CamIDType::CAM_LONGSIDE)
	{
		...
		if (frontCamCheckFLG == enumCamCheck::Check_OK)outInfo.push_back("顶封相机检测OK");
		if (frontCamCheckFLG == enumCamCheck::Check_NG)outInfo.push_back("顶封相机检测 NG");
		if (frontCamCheckFLG == enumCamCheck::Check_NO_CAM)outInfo.push_back("无顶封相机");
	}

	if (myimgInfo2Dlg.camIndex == CamIDType::CAM_SHORTSIDE_1)
	{
		frontCamCheckFLG = getPipelineCamCheck(CamIDType::CAM_SHORTSIDE_0);
		if (frontCamCheckFLG == enumCamCheck::Check_OK)outInfo.push_back("端封相机0检测OK");
		if (frontCamCheckFLG == enumCamCheck::Check_NG)outInfo.push_back("端封相机0检测 NG");
		if (frontCamCheckFLG == enumCamCheck::Check_NO_CAM)outInfo.push_back("无端封相机0");
	}
	if (enumCamCheck::Check_NG == frontCamCheckFLG)
	{
		ret = SaveImga((int)pcamCT->caminfoIndex, sys, &rm);
		if (ret == -3)outInfo.push_back("剩余空间不足2G保存失败");
		if (ret == 0)outInfo.push_back("图片数据保存成功");
		if (ret == -2)outInfo.push_back("图片数据未保存");
		if (ret == -1)outInfo.push_back("未设置图片数据保存");
	...
	else
	{
		...
		sprintf(szInfo, "检测时间 = %ld ms", spendMs);
		outInfo.push_back(szInfo);
	...
	pCtrol->m_onMessage(pCtrol->m_owner, MSG_INFO, (void*)&outInfo, pcamCT->caminfoIndex);
	//pCtrol->LogString(pcamCT->caminfoIndex, outInfo); LOG中丢失时间参数
